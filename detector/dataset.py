...
class Taco(utils.Dataset):
    ...
    def download(self, taco, image_id):

        imag=taco.dataset['images']
        preview=requests.get(imag[image_id]["flickr_url"]).content
        loc="../data/"+imag[image_id]["file_name"]
        with open (loc, 'wb') as handler:
            handler.write(preview)
