...
def compute_matches1(gt_boxes, gt_class_ids, gt_masks,
                    pred_boxes, pred_class_ids, pred_scores, pred_masks,
                    iou_threshold=0.5, score_threshold=0.0):
    """Finds matches between prediction and ground truth instances.
    ...
    # Compute IoU overlaps [pred_masks, gt_masks]
    overlaps = compute_overlaps_masks(pred_masks, gt_masks)
    boxlaps=compute_overlaps(pred_boxes,gt_boxes)
    # Loop through predictions and find matching ground truth boxes
    match_count = 0
    pred_match = -1 * np.ones([pred_boxes.shape[0]])
    gt_match = -1 * np.ones([gt_boxes.shape[0]])
    ...
    return gt_match, pred_match, boxlaps, overlaps
def compute_ap1(gt_boxes, gt_class_ids, gt_masks,
    ...
    """
    # Get matches and overlaps
    gt_match, pred_match, boxlaps, overlaps = compute_matches1(
        gt_boxes, gt_class_ids, gt_masks,
        pred_boxes, pred_class_ids, pred_scores, pred_masks,
        iou_threshold)
    ...
    return gt_match, pred_match, mAP, precisions, recalls, boxlaps, overlaps
